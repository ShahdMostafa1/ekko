  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:'inline-block', background:'rgba(124,92,231,0.15)', border:'1px solid rgba(124,92,231,0.35)', borderRadius:99, padding:'5px 14px', fontSize:15, fontWeight:800, color:'#c4b5fd', letterSpacing:'.1em', marginBottom:18, fontFamily:"'Syne',sans-serif" }}>
          {t('upgradeCta.badge')}
        </div>

        <div style={{ width:72, height:72, borderRadius:22, background:'linear-gradient(135deg, rgba(124,92,231,0.35), rgba(167,139,250,0.15))', border:'1.5px solid rgba(124,92,231,0.4)', boxShadow:'0 0 28px rgba(124,92,231,0.25)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:39, margin:'0 auto 16px', animation:'upgradePulse 2.5s ease-in-out infinite' }}>
          🌊
        </div>

        <h2 style={{ margin:'0 0 8px', fontSize:27, fontWeight:800, color:'#ede9fe', fontFamily:"'Syne',sans-serif", lineHeight:1.25 }}>
          {t('upgradeCta.title')}
        </h2>
        <p style={{ margin:'0 0 18px', fontSize:19, color:'#a78bfa', lineHeight:1.55, fontWeight:500 }}>
        ...
        <ul style={{ listStyle:'none', margin:'0 0 22px', padding:0, textAlign:'left', display:'flex', flexDirection:'column', gap:10 }}>
          {PERKS.map(({ emoji, key }) => (
            <li key={key} style={{ display:'flex', alignItems:'flex-start', gap:10, fontSize:18, color:'#d4c4f0', lineHeight:1.45 }}>
              <span style={{ fontSize:21, lineHeight:1.2, flexShrink:0 }}>{emoji}</span>
        ...
        <div style={{ display:'inline-flex', alignItems:'baseline', gap:4, marginBottom:20, fontFamily:"'Syne',sans-serif" }}>
          <span style={{ fontSize:33, fontWeight:800, color:'#e9d5ff' }}>{t('upgradeCta.price')}</span>
          <span style={{ fontSize:18, color:'#8b7eb8', fontWeight:600 }}>{t('upgradeCta.pricePeriod')}</span>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <button
            ...
            style={{ width:'100%', padding:'14px', background:'linear-gradient(135deg, #6d28d9, #7c5ce7)', border:'none', borderRadius:16, color:'#fff', fontSize:20, fontWeight:800, cursor:'pointer', fontFamily:"'DM Sans',sans-serif", boxShadow:'0 6px 24px rgba(124,92,231,0.4)', transition:'transform .15s, box-shadow .15s' }}
          ...
          <button
            ...
            style={{ width:'100%', padding:'12px', background:'transparent', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, color:'#4b4570', fontSize:18, fontWeight:600, cursor:'pointer', fontFamily:"'DM Sans',sans-serif", transition:'color .2s, border-color .2s' }}
