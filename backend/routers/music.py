...
POST /music/generate        → lyrics + title + Sonauto song (with artist style)
...
def _parse_title_and_lyrics(raw: str) -> tuple[str, str]:
    """Extract TITLE: line and remaining lyrics from model output."""
    ...
    for i, line in enumerate(lines):
        ...
        if stripped.upper().startswith("TITLE:"):
            title = stripped[6:].strip()   # everything after "TITLE:"
            lyrics_start = i + 1
            break

    # Skip blank lines after the title line
    while lyrics_start < len(lines) and lines[lyrics_start].strip() == "":
        lyrics_start += 1

    lyrics = "\n".join(lines[lyrics_start:]).strip()

    # Fallback: use first lyric line as title if model didn't comply
    if not title:
        title = lyrics.splitlines()[0][:40] if lyrics else "Untitled"
...
def _generate_lyrics(
    ...
    system_msg = (
        ...
        "TITLE: <a short poetic song title, 2-5 words, in the same language as the lyrics>\n\n"
        "<lyrics here — no section labels like [Verse] or [Chorus], just raw lyric text>"
    )
    user_msg = (
        ...
        f"Rules:\n"
        f"- First line must be: TITLE: <short poetic title in the lyrics' language>\n"
        f"- Then a blank line\n"
        f"- Then 3 verses, each 3-4 lines, separated by blank lines.\n"
        f"- No explanations, no section labels, just the title line then raw lyrics."
    )
    ...
    for model in OPENROUTER_MODELS:
        try:
            ...
            if content:
                title, lyrics = _parse_title_and_lyrics(content)
                print(f"[music] Lyrics+title via {model} lang={lang_code} title='{title}'")
                return lyrics, title
    ...
    return fallback_lyrics, mood_label or emotion or "Untitled"
...
@router.post("/generate", summary="Generate lyrics + title + Sonauto song with artist style")
async def generate_music(req: GenerateRequest):
    ...
    # _generate_lyrics now returns (lyrics, title)
    lyrics, title = _generate_lyrics(
